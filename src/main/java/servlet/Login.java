...
public class Login extends HttpServlet {
	...
	protected void doPost(HttpServletRequest request, HttpServletResponse response) throws ServletException, IOException {
		...
		if(session.isNew()) {
			...
			if(((String)request.getParameter("user_#")).equals((String) session.getAttribute("user_#"))) {
				System.out.println("이미 로그인 된: " + ++i2);
				System.out.println("이미 로그인 된");
				response.sendRedirect("list.jsp");
			} else {
				System.out.println("다시 로그인: " + ++i3);
	...
	private void sessionCreate(HttpServletResponse response, DAO dao, HttpSession session, String login_id) throws IOException {
		try {
			System.out.println("sessionCreate() :" + ++i6);
			if(dao.isExisted(new String[] {login_id}, Constants.LogedOn)) {
				...
				System.out.println("회원정보가 일치하지 않습니다: " + ++i5);
				System.out.println("회원정보가 일치하지 않습니다."); // -> 추후 alert
				System.out.println(session.getAttribute("user_#"));
				session.invalidate();
